...
Performs Latin Hypercube Sampling (LHS) over 8 physics and biology parameters,
...
| ERR_scale   | normal(1.0, σ=0.30)       |
| EAR_scale   | normal(1.0, σ=0.35)       |
+-------------+---------------------------+
"""
...
PARAM_DESCRIPTIONS: dict[str, str] = {
    ...
    'ERR_scale':  'ERR model scale (normal σ=0.30)',
    'EAR_scale':  'EAR model scale (normal σ=0.35)',
}
...
def lhs_samples(
    ...
    """
    Generate Latin Hypercube samples for the 8 uncertainty parameters.

    Parameters
    ----------
    n_samples   : number of LHS samples
    param_names : subset of PARAM_NAMES to sample; default all 8
    seed        : random seed
    ...
    for idx, pname in enumerate(param_names):
        ...
        if pname in ('phi_scale', 'LIS_norm'):
            sigma = 0.15 if pname == 'phi_scale' else 0.12
            result[pname] = np.exp(norm.ppf(u, loc=0.0, scale=sigma))

        elif pname == 'cross_sec':
            result[pname] = np.exp(norm.ppf(u, loc=0.0, scale=0.20))

        elif pname == 'neutron_H':
            result[pname] = np.exp(norm.ppf(u, loc=0.0, scale=0.25))

        elif pname == 'Q_factor':
            result[pname] = np.exp(norm.ppf(u, loc=0.0, scale=_LN2))

        elif pname == 'DDREF':
            result[pname] = 1.0 + u * 1.0

        elif pname == 'ERR_scale':
            result[pname] = norm.ppf(u, loc=1.0, scale=0.30)

        elif pname == 'EAR_scale':
            result[pname] = norm.ppf(u, loc=1.0, scale=0.35)
...
def _evaluate_one_sample(
    ...
    from gcr.reid import reid_point_estimate
...
def run_uncertainty_ensemble(
    ...
    """
    Run full LHS uncertainty ensemble over 8 physics + biology parameters.
    ...
    samples = lhs_samples(n_samples, seed=seed)
